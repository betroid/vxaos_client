...
module Handle_Data
  ...
  def handle_messages_game(header, buffer)
    ...
    when Enums::Packet::PLAYER_COOLDOWN #LM²
      handle_player_cooldown(buffer)
  ...
  def handle_player_state(buffer) #LM²
    state_id = buffer.read_short
    add_state = buffer.read_boolean
    if add_state
      $game_actors[1].add_state(state_id)
      $game_actors[1].states_time[state_id] = Time.now + buffer.read_float
    else
  ...
  def handle_player_buff(buffer) #LM²
    param_id = buffer.read_byte
    buff_level = buffer.read_short
    if buff_level != 0
      $game_actors[1].buffs_time[param_id] = Time.now + buffer.read_float 
      $game_actors[1].buffs_max_time[param_id] = buffer.read_float
    end
    if buff_level == 1
  ...
  #LM² - Cooldown
  def handle_player_cooldown(buffer)
    id = buffer.read_short
    skill = $data_skills[id]
    $game_actors[1].skills_cooldown_time[id] = Time.now + skill.cooldown
    $windows[:hotbar].reset_cooldown_bar(skill)
  end
...
def handle_event_command(buffer)
  ...
  if event_id > 0
    ...
    if event && event.trigger == 4
      # Evento com condição de início (processo paralelo)
      if event.list
        event_range = event.list[initial_index..final_index] || [] # Garantir que nunca seja nil
        event_last = event.list.last ? [event.list.last] : [] # Verificar explicitamente o último item
        event_commands = event_range + event_last
        event.interpreter.setup(event_commands, event_id)
      else
        puts "Erro: A lista de comandos do evento #{event_id} está vazia."
      end
    elsif event
      # Evento normal
      if event.list
        event_range = event.list[initial_index..final_index] || []
        event_last = event.list.last ? [event.list.last] : []
        event_commands = event_range + event_last
        $game_map.interpreter.setup(event_commands, event_id)
      else
        puts "Erro: A lista de comandos do evento #{event_id} está vazia."
      end
    else
      puts "Erro: Evento #{event_id} não encontrado no mapa."
    end
  else
    # Evento comum (event_id negativo)
    common_event = $data_common_events[event_id.abs]
    if common_event && common_event.list
      event_range = common_event.list[initial_index..final_index] || []
      event_last = common_event.list.last ? [common_event.list.last] : []
      event_commands = event_range + event_last
      $game_map.interpreter.setup(event_commands, event_id)
    else
      puts "Erro: Evento comum #{event_id.abs} está inválido ou não tem lista."
    end
  end
end
