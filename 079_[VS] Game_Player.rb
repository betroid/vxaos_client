...
class Game_Player < Game_Character
  ...
  #LM² - Cooldown
  attr_accessor :skill_cooldown_time
  ...
  def update_trigger
    ...
    # Andrart - Input Action com clique do mouse
    if Mouse.click?(:R) && !$game_map.interpreter.running? && !$typing
      # Verifica se o cursor está sobre um evento válido
  ...
  #LM² - Cooldown
  def use_hotbar
    return if $typing #@item_attack_time > Time.now || $typing 
    actor.hotbar.each_with_index do |item, i|
      next unless item
      next unless Input.trigger?(Configs::HOTKEYS[i])
      if item.is_a?(RPG::Skill)
        if @skill_cooldown_time.has_key?(item.id) && @skill_cooldown_time[item.id] > Time.now
          $error_msg = Vocab::CooldownSkill
          return
        end
      else
        return if @item_attack_time > Time.now
      end
      if usable?(item)
        $network.send_use_hotbar(i) 
        if item.is_a?(RPG::Skill)
          @skill_cooldown_time[item.id] = Time.now + item.cooldown
        else
          @item_attack_time = Time.now + Configs::COOLDOWN_ITEM_TIME
        end
      end
      break
    end
