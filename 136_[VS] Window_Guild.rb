...
class Window_Guild < Window_Selectable
  ...
  def initialize
    super(adjust_x, 230, 252, 222)
    self.visible = false
    self.closable = true
    self.title = Vocab::Guild
    @tab_page = Tab_Control.new(self, [Vocab::Main, Vocab::Manage, "#{Vocab::Member}s"], true) { refresh }
    @shutdown_button = Button.new(self, 180, 192, Vocab.shutdown, 65) { $windows[:choice].show(Vocab::Ask, Enums::Choice::LEAVE_GUILD) }
    @kick_box = Text_Box.new(self, 88, 22, 101, Configs::MAX_CHARACTERS) { enable_kick_button }
    @leader_box = Text_Box.new(self, 88, 48, 101, Configs::MAX_CHARACTERS) { enable_leader_button }
    @notice_box = Text_Box.new(self, 88, 74, 101, 64) { enable_notice_button }
    @kick_button = Button.new(self, 196, 21, Vocab::Ok) { kick_member }
    @leader_button = Button.new(self, 196, 47, Vocab::Ok) { change_leader }
    @notice_button = Button.new(self, 196, 73, Vocab::Ok) { change_notice }
  ...
  def manage_page
    @data.clear
    draw_text(3, 7, 85, line_height, "#{Vocab::Kick}:")
    draw_text(3, 33, 100, line_height, Vocab::NewLeader)
    draw_text(3, 59, 75, line_height, Vocab::Notice)
    ...
    #Noticias
    change_color(text_color(20))
    draw_text(0, 85, contents_width, line_height, 'Visualize a notícia abaixo:', 1)
    change_color(normal_color)
    word_wrap($game_guild.notice).each_with_index do |text, i|
    draw_text(0, line_height * i + 105, contents_width, line_height, text, 1)
  end
